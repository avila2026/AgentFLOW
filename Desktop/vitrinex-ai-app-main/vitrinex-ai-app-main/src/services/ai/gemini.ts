...
export const getGeminiClient = async (explicitKey?: string, userId?: string): Promise<GoogleGenAI> => {
  ...
  // Fallback proxy para interceptar modelos Preview inativos
  ...
  client.models.generateContent = async (params: any) => {
    try {
      ...
      if (msg.includes('404') || msg.includes('not found') || msg.includes('invalid model')) {
        console.warn(`[Fallback] Modelo ${params.model} indisponível. Redirecionando para gemini-2.5-flash...`);
        return await originalGenerateContent({ ...params, model: 'gemini-2.5-flash' });
      }
      throw e;
    }
  };
  ...
  client.models.generateContentStream = async (params: any) => {
    try {
      return await originalGenerateContentStream(params);
    } catch (e: any) {
      const msg = e.message?.toLowerCase() || '';
      if (msg.includes('404') || msg.includes('not found') || msg.includes('invalid model')) {
        console.warn(`[Fallback] Modelo Stream ${params.model} indisponível. Redirecionando para gemini-2.5-flash...`);
        return await originalGenerateContentStream({ ...params, model: 'gemini-2.5-flash' });
      }
      throw e;
    }
  };
...
export const testGeminiConnection = async (explicitKey?: string, userId?: string): Promise<string> => {
  try {
    ...
    const response = await ai.models.generateContent({
      ...
      contents: "Diga 'Sistema VitrineX AI: Operação exclusiva em arquitetura de alto desempenho v3 confirmada.' em português.",
      config: {
