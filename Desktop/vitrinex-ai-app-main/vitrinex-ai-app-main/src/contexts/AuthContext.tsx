...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    useEffect(() => {
        ...
        const initAuth = async () => {
            // 🧪 DEV BYPASS — acesso sem login para testes.
            // Ativado por VITE_DEV_BYPASS_AUTH=true no .env. NUNCA usar em produção.
            if (import.meta.env.VITE_DEV_BYPASS_AUTH === 'true') {
                console.warn('🧪 AUTH BYPASS ATIVO — entrando como usuário de teste (Admin). Desative VITE_DEV_BYPASS_AUTH no .env para produção.');
                const devUser: User = {
                    id: 'dev-bypass-admin',
                    app_metadata: {},
                    user_metadata: { full_name: 'Modo Teste (Admin)' },
                    aud: 'authenticated',
                    created_at: new Date().toISOString(),
                    email: 'teste@vitrinex.local',
                };
                setUser(devUser);
                setSession({
