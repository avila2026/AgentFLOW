        ...
        const fetchDynamicPlaceholders = async () => {
            try {
                ...
                const response = await ai.models.generateContent({
                    ...
                    contents: 'Generate 20 creative, short, diverse UI component prompts (e.g. "bioluminescent task list") in Portuguese (Brazil). Return ONLY a raw JSON array of strings. IP SAFEGUARD: Avoid referencing specific famous artists, movies, or brands.',
                });
...
Return ONLY RAW HTML. No markdown fences.
    ...
    return (
        <div className="flash-ui-root relative w-full h-full bg-[#09090b] text-white font-sans overflow-hidden">
            <a href="https://x.com/ammaar" target="_blank" rel="noopener noreferrer" className={`creator-credit ${hasStarted ? 'hide-on-mobile' : ''}`}>
                    ...
                    <div className={`input-wrapper ${isLoading ? 'loading' : ''}`}>
                        ...
                        {!isLoading ? (
                            <input
                                ref={inputRef}
                                type="text"
                                aria-label="Descreva a interface que você quer desenhar"
                                placeholder={placeholders[placeholderIndex] || "Descreva a interface que você quer desenhar"}
                                value={inputValue}
                        ...
                        <button className="send-button" onClick={() => handleSendMessage()} disabled={isLoading || !inputValue.trim()} aria-label="Enviar prompt para geração">
                            <ArrowUpIcon />
                        </button>
                    </div>
                </div>
